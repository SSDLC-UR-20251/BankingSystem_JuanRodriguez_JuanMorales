...
Session(app)  # Inicializa Flask-Session correctamente
...
def read_record():
    ...
    # Creamos una copia para modificar sin afectar el original
    db_copia = db.copy()
    ...
    for email, data in db_copia.items():
        ...
        if dni and nonce:  # Verifica que existen los valores antes de usarlos
            dni_of = cuatro_digitos(dni, nonce, clave)  
            db_copia[email]["dni"] = dni_of  # Actualiza el valor en la copia
...
def api_withdraw():
    ...
    if igual:
        ...
        return redirect(url_for('customer_menu', message="Retiro no autorizado: Contraseña incorrecta"))
